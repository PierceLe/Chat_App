...
import { getAllFriends, UserData } from "@/core/services/contactService";
...
const GroupChat = () => {
  ...
  return (
      <>
                      ...
                      <ul className="dropdown-menu dropdown-menu-end p-3">
                        ...
                        <li>
                          <Link
                              to="#"
                              className="dropdown-item"
                              onClick={() => setIsRenameGroupModalVisible(true)}>
        ...
        {/* Add Member Modal */}
        <Modal title="Add Members to Group"
                         ...
                         onClick={async () => {
                           try {
                             if (selectedMembersToAdd.size === 0) {
                               notify.error('No members selected', 'Please select at least one member to add');
                               return;
                             }
                             ...
                             if (response.code === 0) {
                               ...
                               const errorMessage = response?.data?.message || 'Failed to add members';
                               notify.error('Error', errorMessage);
                             }
                           } catch (error: any) {
                             console.error(error);
                             notify.error('Failed to add members', 'Please try again later');
                           }
                         }}>
                   Add Members
                 </Button>,
          ...
          <div style={{ marginBottom: 16 }}>
            <Input.Search placeholder="Search friends" style={{ marginBottom: 8 }} />
          </div>
          <List
                ...
                return (
                    <List.Item>
                      <div style={{display: 'flex', alignItems: 'center', width: '100%'}}>
                        ...
                        {isAlreadyMember ? (<span style={{ color: 'green' }}>Already a member</span>)
                            : (<Checkbox checked={selectedMembersToAdd.has(friend.user_id)}
                                         onChange={(e) => {
                                           const newSelected = new Set(selectedMembersToAdd);
                                           if (e.target.checked) {
                                             newSelected.add(friend.user_id);
                                           } else {
                                             newSelected.delete(friend.user_id);
                                           }
                                           setSelectedMembersToAdd(newSelected);
                                         }}/>)}
                      </div>
                    </List.Item>
                );
